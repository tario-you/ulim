...
def evaluation_vqvae(out_dir, val_loader, net, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, eval_wrapper, draw = True, save = True, savegif=False, savenpy=False) : 
    ...
    for batch in tqdm(val_loader):
        ...
        if torch.cuda.is_available():motion = motion.cuda()
        et, em = eval_wrapper.get_co_embeddings(word_embeddings, pos_one_hots, sent_len, motion, m_length)
        bs, seq = motion.shape[0], motion.shape[1]

        num_joints = 21 if motion.shape[-1] == 251 else 22
        
        pred_pose_eval = torch.zeros((bs, seq, motion.shape[-1]))
        if torch.cuda.is_available(): pred_pose_eval = pred_pose_eval.cuda()

        for i in range(bs):
            pose = val_loader.dataset.inv_transform(motion[i:i+1, :m_length[i], :].detach().cpu().numpy())
            if torch.cuda.is_available(): pose_xyz = recover_from_ric(torch.from_numpy(pose).float().cuda(), num_joints)
            else: pose_xyz = recover_from_ric(torch.from_numpy(pose).float(), num_joints)


            pred_pose, loss_commit, perplexity = net(motion[i:i+1, :m_length[i]])
            pred_denorm = val_loader.dataset.inv_transform(pred_pose.detach().cpu().numpy())
            if torch.cuda.is_available(): pred_xyz = recover_from_ric(torch.from_numpy(pred_denorm).float().cuda(), num_joints)
            else: pred_xyz = recover_from_ric(torch.from_numpy(pred_denorm).float(), num_joints)
...
def evaluation_transformer(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, draw = True, save = True, savegif=False) : 
    ...
    for i in range(1):
        for batch in val_loader:
            ...
            if torch.cuda.is_available(): text = clip.tokenize(clip_text, truncate=True).cuda()
            else: text = clip.tokenize(clip_text, truncate=True)

            feat_clip_text = clip_model.encode_text(text).float()
            if torch.cuda.is_available(): pred_pose_eval = torch.zeros((bs, seq, pose.shape[-1])).cuda()
            else: pred_pose_eval = torch.zeros((bs, seq, pose.shape[-1]))
            ...
            for k in range(bs):
                try:
                    ...
                    if torch.cuda.is_available(): index_motion = torch.ones(1,1).cuda().long()
                    else: index_motion = torch.ones(1,1).long()
            ...
            if i == 0:
                if torch.cuda.is_available(): pose = pose.cuda().float()
                else: pose = pose.float()

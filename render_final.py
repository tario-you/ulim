...
def render(motions, outdir='test_vis', device_id=0, name=None, pred=True):
    ...
    device_available = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    rot2xyz = Rotation2xyz(device=device_available)
...
if __name__ == "__main__":
    ...
    for i in range(1,3997):
        filename = f"/0{str(i).zfill(4)}.npy"
        # for filename in filename_list:
        motions = np.load(filedir+filename)
        print(motions.shape)

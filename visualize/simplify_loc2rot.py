...
class joints2smpl:
    ...
    def npy2smpl(self, npy_path):
        ...
        print(f"the{npy_path=}")
        motions = np.load(npy_path, allow_pickle=True)[None][0]
        print(motions)
        n_samples = motions.shape[0] # ['motion']
        all_thetas = []
        for sample_i in tqdm(range(n_samples)):
            print(motions[sample_i].shape)
            # print(self.joint2smpl(motions[sample_i].transpose(2, 0, 1)).shape)
            # quit()
            thetas, _ = self.joint2smpl(motions[sample_i].transpose(2, 0, 1))  # [nframes, njoints, 3] # ['motion']
            all_thetas.append(thetas.cpu().numpy())
        motions = np.concatenate(all_thetas, axis=0) # ['motion']
        print('motions', motions.shape) # ['motion']
...
if __name__ == '__main__':
    ...
    # Load the .npy file
    data = np.load(params.input_path)
    if "vec" not in params.input_path:
        frames, _, _ = data.shape
    else:
        frames, _ = data.shape

    simplify = joints2smpl(frames, device_id=params.device, cuda=params.cuda)

...
##### ---- Network ---- #####
clip_model, clip_preprocess = clip.load("ViT-B/32", device=device_available, jit=False)  # Must set jit=False for training
...
trans_encoder.train()
if torch.cuda.is_available(): trans_encoder.cuda()
...
for batch in train_loader_token:
    ...
    if torch.cuda.is_available(): pose = pose.cuda().float() # bs, nb_joints, joints_dim, seq_len
...
while nb_iter <= args.total_iter:
    ...
    if torch.cuda.is_available(): m_tokens, m_tokens_len = m_tokens.cuda(), m_tokens_len.cuda()
    bs = m_tokens.shape[0]
